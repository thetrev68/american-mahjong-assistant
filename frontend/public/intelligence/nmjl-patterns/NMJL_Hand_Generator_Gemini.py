...
def generate_playable_hands(patterns):
    ...
    for hand_key, sub_patterns in pattern_map.items():
        ...
        for pattern in sub_patterns:
            ...
            # Generate the primary suit combinations based on the number of unique roles
            if len(suit_roles_needed) == 0:
                # No primary suit roles, so only one hand variation
                base_combinations = [()]
            elif len(suit_roles_needed) == 1:
                # One primary suit role ('any'), so three variations
                base_combinations = [(s,) for s in SUITS]
            elif len(suit_roles_needed) == 2:
                # Two primary suit roles ('any', 'second'), so six permutations
                base_combinations = list(itertools.permutations(SUITS, 2))
            else: # len(suit_roles_needed) == 3
                # Three primary suit roles ('any', 'second', 'third'), so six permutations
                base_combinations = list(itertools.permutations(SUITS, 3))
