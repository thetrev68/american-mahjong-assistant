...
const HostControls: React.FC = () => {
  const { room, isHost, players } = useRoomStore((s) => ({
    room: s.room,
    isHost: s.isHost,
  ...
  const handleUpdateSettings = () => {
    if (!roomService || !roomSettings) return;

    roomService.updateRoomSettings(roomSettings);
  ...
  return (
    <Card className="p-4">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <Button onClick={() => setShowSettings(true)} variant="secondary" size="sm" disabled={!isHost}>
          Room Settings
        </Button>
        <Button onClick={() => setShowTransferHost(true)} variant="secondary" size="sm" disabled={!isHost || nonHostPlayers.length === 0}>
          Transfer Host
        </Button>
      </div>
      ...
      <div className="space-y-2">
        ...
        {nonHostPlayers.map((player: any) => (
          <div key={player.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
            ...
            <Button onClick={() => handleKickPlayer(player.id)} variant="secondary" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50" disabled={!isHost}>
              Kick
            </Button>
          </div>
        ))}

        {nonHostPlayers.length === 0 && <p className="text-sm text-gray-500 text-center py-2">No other players to manage</p>}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={resetSettings}>
          <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              {/* Turn Time Limit */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Turn Time Limit (seconds, 0 = no limit)</label>
                <input
                  type="number"
            ...
            <div className="flex space-x-3 mt-6">
              <Button onClick={handleUpdateSettings} variant="primary" className="flex-1">Update Settings</Button>
              <Button onClick={resetSettings} variant="secondary" className="flex-1">Cancel</Button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setShowTransferHost(false)}>
          <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
              ...
              {nonHostPlayers.map((player: any) => (
                <label key={player.id} className="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {!player.isConnected && <span className="text-xs text-red-600">(Disconnected)</span>}
                  </div>
                </label>
              ))}
            </div>
            ...
            <div className="flex space-x-3">
              <Button onClick={handleTransferHost} variant="primary" className="flex-1" disabled={!selectedNewHost}>Transfer Host</Button>
              <Button
                onClick={() => {
                  setShowTransferHost(false);
                  setSelectedNewHost('');
